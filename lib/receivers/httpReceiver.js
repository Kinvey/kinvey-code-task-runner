...
module.exports = (() => {
  ...
  function addFunctionsTaskAttributes(req, res, next) {
    ...
    req.task.request.objectName = req.get('X-Kinvey-Object-Name') || '';
    next();
  }
  ...
  function checkIfFlexFunction(req, res, next) {
    if (req.params.serviceObjectName === '_flexFunctions') {
      next('route');
    } else {
      next();
    }
  ...
  function startServer(taskReceivedCallback, startedCallback, options) {
    ...
    // FlexFunctions route
    router.post('/_flexFunctions/:handlerName', generateBaseTask, addFunctionsTaskAttributes, appendQuery, appendId, appendBody, sendTask);
