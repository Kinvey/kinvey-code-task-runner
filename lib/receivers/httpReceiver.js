...
module.exports = (() => {
  ...
  function addLogicTaskAttributes(req, res, next) {
    req.task.taskType = 'businessLogic';
    req.task.request.collectionName = req.params.objectName;
    next();
  }

  function addDataTaskAttributes(req, res, next) {
    req.task.taskType = 'dataLink';
  ...
  function discover(req, res, next) {
    ...
    return res.status(500).json(req.task);
  }
  ...
  function startServer(taskReceivedCallback, startedCallback, options) {
    ...
    router.post('/:serviceObjectName/', generateBaseTask, addDataTaskAttributes, appendBody, sendTask);
    ...
    router.post('/:objectName/:handlerName', generateBaseTask, addLogicTaskAttributes, appendQuery, appendId, appendBody, sendTask);

    router.post('/command/discover', discover);
